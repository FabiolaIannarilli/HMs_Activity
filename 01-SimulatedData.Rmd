...
p_df$curvesLeg = as.factor(rep(c("+/- 0.5 SD", "+/- 1 SD", "+/- 1.5 SD", "Conditional", "+/- 0.5 SD", "+/- 1 SD", "+/- 1.5 SD"), each = length(time)))
...
p_df$curvesLeg = as.factor(rep(c("+/- 0.5 SD", "+/- 1 SD", "+/- 1.5 SD", "Conditional", "+/- 0.5 SD", "+/- 1 SD", "+/- 1.5 SD"), each = length(time)))
p_df$mean <- c(rep(rep("off", length(time)),3), rep("on", length(time)), rep(rep("off", length(time)),3))# change in frequency of site-use
...
for(i in 1:length(time)){
  intfun<-function(gamma_x){
     plogis(b0 + b1*cos(2*pi*time[i]/(24)+ theta0 + gamma_x) +
              tau_i[4]) * dnorm(gamma_x, mean = 0, sd = 1)
       }
       p_marg[i]<-integrate(intfun,-Inf, Inf)[1]
}
p_df2 <- data.frame(p = unlist(p_marg),
...
This second example clearly illustrates how varying $\gamma_i$ (while keeping all the other parameters constant) leads to the horizontal shift of the curves, with peaks in activity equal across time but occurring at different times of the diel cycle.

In the next chapter, we simulate data drawing values of $\tau_i$ and $\gamma_i$ from the normal distributions specified at the beginning of this chapter. We use these simulated data to illustrate how trigonometric and cyclic cubic regression spline HMs can be used to estimate activity patterns.<!--- In chapter \@ref(condvsmarg), we illustrate the difference between conditional and marginal mean activity patterns and discuss when one might choose one curve over the other depending on one's ecological questions.--->
