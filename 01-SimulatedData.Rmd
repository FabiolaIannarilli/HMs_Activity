...
We discuss differences between conditional and marginal means in detail in Tutorial \@ref(condvsmarg) and in Box 1 in the main text [@HMsActivity2024]. Although we can consider conditional means for any site, it is often instructive to consider the conditional mean curve for a "typical site" obtained by setting all the random effects equal to zero; it is this conditional mean we are referring to if we do not mention a specific site. We can plot the conditional and marginal curves using the code below:
