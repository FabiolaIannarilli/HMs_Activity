...
In this first example based on real data, we illustrate how trigonometric GLMMs can be used to describe different structures of diel activity patterns. Traditionally, diel activity patterns are usually classified as:
...
dat <- read.csv("data_input/AllRecords_allSpecies_allSessions_0min_deltaT.csv", as.is = TRUE) %>% 
  filter(c(Species == "Coyote" ) & Station != ".dtrash") %>% droplevels %>%  
...
Both comparisons show that the XXXX is the most supported model among those compared. We can, thus, conclude that coyotes have a XXX activity pattern.
...
``` {r univsbim7}
# build a new dataset 
newdat <- with(occasions_bind, expand.grid(Time = seq(min(Time), max(Time), length.out = 24)))
