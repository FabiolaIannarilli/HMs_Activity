...
To test the importance of the seasonal effect on activity patterns of black bears, we can also compare the model above with a model that does not include the covariate using AIC: 
...
(ggarrange(pl_trig, pl_cycl, ncol = 2, common.legend = TRUE, legend = "bottom", widths = c(0.33, 0.66)) +
  theme(plot.margin = margin(0.1,0.1,0.5,0.1, "cm")))
```

In this analysis, the patterns estimated by the two models structure is close to identical, suggesting there is minimal varation in timing of activity peaks among sites.

As done for the trigonometric models, we can test the importance of the variable _Season_ by comparing the model without a global smoother with a similar model that does not include this covariate using AIC.
...
```
and then proceed with the arranging the data in the format required to run the HM-based models, as we have done before in other examples.  
