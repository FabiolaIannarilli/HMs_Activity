...
The patterns estimated by the cyclic cubic spline model also closely matches that returned by the random intercept-only trigonometric model. In \@ref(compkde), we repeated this analysis using KDEs (and aggregated data) to compare estimates based on the HMs and KDEs approaches.

**Model selection.** As done for the trigonometric models, we can use AIC to test the importance of the variable *Season* by comparing the model without a global smoother with a similar model that does not include this covariate. Importantly, (generalized) LRTs are not recommended when comparing HGAMs and there are some known issues when comparing HGAM models with and without global smoother using AIC (see @Pedersen2019). We thus recommend caution and stress that a careful a priori choice of modeling structures based on the desired inference and the characteristics of the data is often preferrable.  
...
We can extend the previous model structure to accommodate site-to-site variability in timing of activity peaks by adding a smoother for _Time_ that depends on _Site_ and a global smoother for _Time_. This second structure mimics a random intercept and random slope model.
...
(cycl_pred %>% filter(Site %in% selected_sites) %>% 
...
ggsave(filename = "ms_figures/Fig4_Site_specific_estimates.jpg", device = "jpeg", units = "cm", width = 24, height = 20, dpi = 600)

```

The second structure has an additional term, a general smoother for *Time*, and resembles a trigonometric GLMM with random intercept and slope.
