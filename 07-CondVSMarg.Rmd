...
where $\beta_0$, $\beta_1$, $\beta_2$, $\theta_0$, and $\theta_1$ correspond to the parameter values used to simulate the data (see chapter \@ref(est)).
...
Before fitting models, we need to prepare the data. We use the code from chapter \@ref(est) to aggregate the data by site, resulting in a data set with the following variables:
...
We start by fitting the model, similar to how we have done it in previous chapters of this tutorial.
