...
```{r setsim5}
mod_rand_int <- mixed_model(fixed = cbind(success, failure) ~ 
...
cond_eff0 <- effectPlotData(mod_rand_int, newdat, marginal = FALSE) %>% 
...
```{r setsim7}
mod_rand_slope <- mixed_model(fixed = cbind(success, failure) ~ 
                      cos(2 * pi * Time/24)  + sin(2 * pi * Time/24) +
                      cos(2 * pi * Time/12)  + sin(2 * pi * Time/12),
...
# estimated activity pattern
newdat <- with(occasions_cbind, expand.grid(Time = seq(min(Time), 24, length.out = 48)))
cond_eff1 <- effectPlotData(mod_rand_slope, newdat, marginal = FALSE) %>% 
  mutate(pred = plogis(pred),
         low = plogis(low),
         upp = plogis(upp),
