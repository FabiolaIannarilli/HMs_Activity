...
* whether a certain species of interest concentrates its activity at one or two specific times of the day (e.g. unimodal and bimodal patterns), or whether its activity is constant throughout the 24-hour cycle (cathemeral pattern; chapter \@ref(unibim));
...
* whether changes in diel activity are linked to the co-occurrence of another species, with potential evidence for the existence of time-partitioning mechanisms (chapter \@ref(temppart)).

The tutorial also includes a description of the difference between conditional and marginal mean activity patterns (chapter \@ref(condvsmarg)), how we can obtain both means and how this difference is relevant depending on the ecological questions at hand. We also provide a chapter focused on Kernel Density Estimators (KDEs), currently the most used approach for the estimate of activity patterns (chapter \@ref(kdes)). We compare the three R-packages currently available to estimate diel activity using KDEs (section \@ref(kdespkg)), illustrate the consequences of non-aggregating the data in independent events when estimating diel activity (section \@ref(kdedataaggr)), and close the chapter with a comparison of estimates of activity based on KDEs and hierarchical models (section \@ref(compkde)).

In the Appendix \@ref(app), we provide several additional resources, including:
...
3\. an exmaple of how to apply these models in a Bayesian framework (section \@ref(bay));
