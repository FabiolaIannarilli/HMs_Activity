...
```{r temp1, message = TRUE, warnings = TRUE}
# Load libraries
rm(list = ls())
set.seed(129)
library(dplyr) 
...
Wolves and coyotes have been detected at `r nrow(site_Wo)` and `r nrow(site_Co)` site-sessions, respectively, `r nrow(site)` of which were the same.
...
Then, we populate these dataframes with the information recorded for coyote and wolf, respectively. We start with coyotes.
...
We now proceed to fit a model for each species. For convenience, we run a trigonometric random intercept-only model, but more complex trigonometric GLMMs or cyclic cubic spline HGAMs can also be used, as we show below.
...
We now plot the results for both coyotes and wolves in the presence and absence of the other species. We focus on estimating marginal mean activity patterns because our aim is to compare between two groups that differ in their characteristics (i.e. site-sessions with and without the other species). The only change to the code required to obtain the marginal means is to set the argument _marginal_ in the _effectPlotData_ function equal to _TRUE_.
...
This figure and a comment on the results are presented in @Iannarilliea2024 (Figure 5). Here we proceed instead with addressing the same question using a cyclic cubic spline HGAM (without a site-specific smoother) for each species and then plotting the estimates of activity patterns to visually inspect the results.
...
*Model selection.* For each of the two species, we can now compare the models that include and do not include the presence of the other species. For coyotes,

```{r temp11}
print("Coyote")
print("Trigonometric GLMMs:")
...
A similar conclusion is reached for the wolf. These results do not imply a casual effect, but rather define an association: in site-sessions when the species cooccur (i.e. in space and time), they tend to have activity patterns that differ compared to site-sessions when they do not cooccur. These differences might be driven by other factors (e.g., level of anthropization) not considered in the analysis.
