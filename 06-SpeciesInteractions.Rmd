...
As an example, we analyze temporal partitioning between coyotes and wolves. We format the data as usual.  
...
dat <- read.csv("data_input/AllRecords_allSpecies_allSessions_0min_deltaT.csv", as.is = TRUE) %>% 
  filter(c(Species == "Coyote" | Species == "Wolf") & Station != ".dtrash") %>% droplevels %>%  
...
```
However, before organizing the data using the _cbind(success, failure)_ approach we need to additional information on whether the other species was also detected at least once at a certain site during a sampling session. We thus create two lists of _site X session_ (hereafter, site-session) in which either wolves or coyotes have been encountered and then look at the overlap between these two lists. 
...
```
Wolves and coyotes have been detected at `r nrow(site_Wo)` and `r nrow(site_Co)` site-session, `r nrow(site)` were the same.
...
occasions_Co_cbind <- occasions_Co %>% 
  ...
  summarize(success = sum(capt),
            failure = n() - success)
```
We had 127 hourly intervals (out of 56 928) and 115 hourly intervals (out of 514 133) in which coyotes were recorded at a site-session where wolves were and were not photographed.
...
```
We repeate the same steps for wolves. For this species, we had 176 (out of 56 928) and 348 (out of 514 248) hourly intervals with at least 1 record of wolves at site-sessions in which coyotes were and were not recorded.
