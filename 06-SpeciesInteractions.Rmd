...
Another common question related to activity patterns often asked by ecologists is whether a species changes its activity due to the co-occurrence of another species. In this section, we illustrate how HMs can be used to also address this set of questions. 

As an example, we analyze temporal partitioning between coyotes and wolves (_Canis lupus_). We format the data as usual.  
...
Then, we populate this dataframes with the information recorded for coyote and wolf, respectively. We start with coyotes.
...
We now plot the results for both coyotes and wolves in the presence and absence of the other species.
...
```
This figure and a comment on the results are presented in the main manuscript (Figure 3). Here we proceed instead with addressing the same question using a cyclic cubic spline HGAM (without general smoother) for each species and then plotting the estimates of activity patterns to visually inspect the results.
...
ggarrange(pl_trig, pl_cycl, nrow = 2, ncol = 1, common.legend = TRUE, legend = "right") %>% 
ggsave(filename = "ms_figures/Fig4_Coyote_Wolf_patterns.jpg", device = "jpeg", units = "cm", width = 24, height = 20, dpi = 600)
```
...
*Model selection.* For each of the two species, we can now compare the models that include and do not include the presence of the other species. 
For coyotes, 
``` {r temp11}
print("Coyote")
