...
In the previous chapters, we use simulated data to introduce the concept of variability in activity patterns (chapter \@ref(var)) and to illustrate the use of HM-based approaches for their estimate (chapter \@ref(est)). We now switch gear. In this and the following three chapters, we fit trigonometric and cyclic cubic spline HMs to real camera-trap data to answer some of the most common ecological questions related to the study of activity patterns.

In this chapter we explain how to prepare the data to analyze activity patterns from camera-trap datasets using HMs. This procedure is applied throughout the tutorial every time we use a new set of real data. To minimize redundancy, we illustrate the procedure in details here only once, and then refer to and extend on the code below, when necessary. 

We start by loading the data and showing a preview of what they look like. Here, we select camera-trap records of coyotes (*Canis latrans*). We also load some libraries to wrangle the data in the desired format.

```{r prep1, message = TRUE, warnings = TRUE}
# Load libraries
rm(list = ls())
set.seed(129)
library(dplyr)
...
Then we create a dataframe to store the information about when and when not the target species was encountered at the different locations and within temporal occasions of length of our choice. Here, we use temporal occasions 60 minutes long, but different durations (from seconds to several hours) can be used. Shorter lengths will increase computational time and could potentially lead to convergence problems if data are too sparse. In the code below, the argument _by = '60 min'_ can be changed to accommodate intervals of length other than 1 hour (e.g. _by = '30 min'_ when considering interval 30-min long). 

For each site, we create a list of all the hourly occasions from the start to the end of the deployment of an active camera at that site. We then prepopulate the column _capt_ (which will track the encounter/non-encounter information) with zeros. 
...
For each record of coyote collected during the study, we assign the value 1 at the column _capt_ in the corresponding row for the combination of site and time interval that includes the time of the encounter event. 
