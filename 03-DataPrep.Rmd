...
In the previous sections, we use simulated data to introduce the concept of variability in activity patterns \@ref(var) and to illustrate the use of GLMM-based approaches for their estimate \@ref(est). We now switch gear. In this and the following how-to sections, we use real camera-trap data and answer some of the most common ecological questions related to the study of activity patterns using trigonometric and cyclic cubic spline GLMMs.
...
The next two sections explain how to prepare the data to analyze activity patterns from camera trap data. These steps will be applied every time we use a new set of data in the different examples presented in the next how-to sections. To minimize redundancy, we illustrate these processes in details only once, and then refer to and extend on the code below, when necessary. 
...
Comparing the resulting number of records per location with and without data aggregation shows how most of images were taken within less than 30 minutes from the previous. In the left panel, each record corresponds to an image, whereas, in the right panel, each record corresponds to an independent encounter event. In our specific case, aggregating data also removes duplicates recorded within the same trigger event. We will use the aggregated version of the dataset in the next sections of this tutorial.
...
For each site, we first create a list of all the hourly occasions from the start to the end of the deployment of an active camera at that site. We then prepopulate the column _capt_ (which will track the encounter/non-encounter information) with zeros.
...
We have 251 and 570925 1-hour long occasions with and without encounters of coyotes, respectively. There is a minor mismatch between the occasions with encounters (251), and the number of independent encounters (253). This is due to some independent encounters are more than 30 minute apart (time threshold used to deifne independency) but still fall within the same time interval (if their datetime values are, for example, '2021-04-15 13:06:08' and '2021-04-15 13:46:09').
