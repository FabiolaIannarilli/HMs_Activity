...
As we mentioned in chapter \@ref(est), it is not possible to directly model activity patterns as specified in equation 1 in section \@ref(var) (and in the main manuscript) due to the lack of ready-to-use tools to fit non-linear models in a frequentest framework. Working in the Bayesian framework allows analysts to overcome this limitation.

Here, we use simulated data to illustrate how to fit Bayesian versions of the trigonometric and cylic cubic spline models. 
...
We simulate data using the same approach and the same parameters presented in section \@ref(est). As such, the simulated data are generated from the same 'true' activity patterns. 
...
```
We now create a matrix, _time_, that specifies the time of each interval and that has the same dimensions of the matrix *sim_data* containing the simulated hourly encounter/nonencounter events.
...
We can now specify a model that exactly mirrors the structure of equation 1. To note that we incorporated the site-level random intercept $\tau_i$ into the intercept $\beta_0$.  

``` {r bay3}
# Fabiola cuts code that is not implemented here or documents whtehr these are useful alternatives
# Specify model
Act_bay <-nimbleCode({
...
We now need to specify the initial values and MCMC settings and run the model using _nimble_ [@nimble1, @nimble2, @nimble3].

``` {r bay4, cache=TRUE, cache.path="cache/", eval=FALSE}
# MCMC settings
...
```{r bay.cache, eval=TRUE, echo=FALSE}
#knitr::load_cache("bay4")
#Fabiola removes this and uses the cache approach?
load("res.nimble")
```
...
### Plot Predictions
``` {r bayend, eval = TRUE, class.source = 'fold-hide'}
par(mfrow=c(2,2))
# TRUE
...
To optimize computational time, we can reduce the number of rows in the dataset by counting the number of hourly-long occasions in which the species was observed (i.e. successes) and the number of occasion in which the species was not observed (i.e. failures) for each hourly time interval and camera site (see also \ref{est5}).
...
We are now ready to setup the Bayesian cyclic cubic spline model using similar code as \ref{est10}. However,
here we wrap our code in the brm function from the package brms. We also need to change our notation slightly by specifying the number of successes
and failure as 'success| trial(add variable here)'. From the brms R package vignette, "cbind() syntax will not work in brms in the expected way because this syntax is reserved for other purposes." Note that we only considering random intercepts, but we can include random slope terms also by addiding this to our model definition: s(Time, Site, bs="re").

Something also to consider is how we go about fitting the random effect term in brms. The main consequenes of how we go about doing this is whether we can easily estimate the conditional mean or not. Note that brms functions do not allow excluding smooth terms, such as (Site, bs="re") when making predictions ([see here](https://discourse.mc-stan.org/t/excluding-a-smooth-term-in-brms-using-posterior-linpred/10960/4)). We can consider a slightly different parameterization than \ref{est10}, which makes it straightfrward in estimating the conditional mean . Let's consider both approaches...


```{r fit.bayes.cc.3, cache=TRUE, cache.path="cache/", eval=FALSE, echo=TRUE}
...
# Now fit the model
  cycl_rand_int <- update(cycl_rand_int_setup, recompile = FALSE, 
                          newdata = occasions_cbind,
                          chains = 3)
  
  cycl_rand_int_alt <- update(cycl_rand_int_setup_alt, recompile = FALSE, 
                              newdata = occasions_cbind,
                              chains = 3)
```

```{r fit.bayes.load, eval=TRUE, echo=FALSE}
#Fabiola removes this and uses the cache approach?
load("cycl_rand_int")
load("cycl_rand_int_alt")
```
...
First, let's predict the site-level variability

```{r fit.bayes.cc.pred1}
# Predict the mean site-level activity pattern
  newdata = data.frame(Time=0:23,Site = occasions_cbind$Site, n_events=1)
...
Next, we can predict the marginal mean activity curve by averaging across the site-level variation.

```{r fit.bayes.cc.pred2, class.source = 'fold-hide'}
...
Next, lets predict the conditional mean activity. The benefit of the cycl_rand_int_alt parameterization is simply to conveniently estimate the conditional mean activity curve. Note how the first model structure with a random effect spline has no random effects
according to brms. However, the second model structure does..
...
Because brms does not allow smooth terms, such as (Site, bs="re"), to be omitted when predicting, we will use the second model to predict the conditional mean activity curve. 

```{r fit.bayes.cc.predict3, class.source = 'fold-hide'}
# Predict the conditional mean activity
  newdata2 = data.frame(Time=0:23,n_events=1)
  preds2=fitted(cycl_rand_int_alt,scale="response", newdata=newdata2,re_formula = NA)
...
``` {r bay4, eval = FALSE, cache=TRUE, cache.path="cache/"}
# Other settings
inits <- function() list(beta0 = runif(nrow(sim_data), 0, 3), theta0 = runif(nrow(sim_data), 0, 2*pi),
                         theta1 = runif(nrow(sim_data), 0, 2*pi))
params <- c("mu.beta0", "sd.beta0", "beta1", "beta2", "mean.theta0", "mean.theta1", "sd.theta")
