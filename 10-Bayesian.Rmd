...
We are now ready to setup the Bayesian cyclic cubic spline model using similar code as \ref(est10). However, here we wrap our code in the `brm` function from the package `brms` [@brms]. We also need to change our notation slightly by specifying the number of successes and failure as `success| trial(add variable here)`. From the `brms` R package vignette, "cbind() syntax will not work in `brms` in the expected way because this syntax is reserved for other purposes." Note that we only considering random intercepts, but we can include random slope terms also by addiding this to our model definition: `s(Time, Site, bs="re")`.

Something also to consider is how we go about fitting the random effect term in `brms`. The main consequences of how we go about doing this is whether we can easily estimate the conditional mean or not. Note that `brms` functions do not allow excluding smooth terms, such as `s(Site, bs="re")` when making predictions ([see here](https://discourse.mc-stan.org/t/excluding-a-smooth-term-in-brms-using-posterior-linpred/10960/4)). We can consider a slightly different parameterization than \ref(est10), which makes it straightforward in estimating the conditional mean. Let's consider both approaches...
...
Next, lets predict the conditional mean activity. The benefit of the `cycl_rand_int_alt` parameterization is simply to conveniently estimate the conditional mean activity curve. Note how the first model structure with a random effect spline has no random effects according to `brms`. However, the second model structure does..
...
# Plot the onditional mean and compare it to the simulated true mean
