...
Activity patterns are currently estimated primarily using Kernel Density Estimators (KDEs). However, this approach presents several limitations that we highlight in @Iannarilliea2024. In this chapter, we introduce two alternative, model-based approaches to estimate activity patterns from time stamped data:
...
Both types of hierarchical models (HMs) can accommodate the periodic nature of activity data, and they can both also address site-to-site and other forms of variability. Although our examples are all focused on the analysis of camera-trap data, the same framework can be applied to data collected using other static devices (e.g., acoustic recorders) or site-based sampling strategies.

## Simulating activity patterns

To show how trigonometric and cyclic cubic spline HMs can successfully recover the true pattern of activity, we apply these approaches to simulated data. As such, we can compare the estimated patterns to the known, 'true', simulated patterns.

To keep the focus on estimating activity patterns, we encapsulate the code to simulate the data in the function `sim_activity`, that we source after loading the necessary libraries. This function simulates data based on equation 1 (chapter \@ref(var) and Materials and Results in @Iannarilliea2024). The function, including a description of its arguments and outputs, is provided in chapter \@ref(app).  
...
We simulate site-specific bimodal activity patterns for 100 sites (M) and 30 days (J) using the parameters defined in the code below. We include both variability in frequency of site use and variability in timing of activity peaks by drawing values of $\tau_i$ and $\gamma_i$ from the distributions $\tau_i \sim N(0,\sigma_\tau)$ and $\gamma_i \sim N(0,\sigma_\gamma)$, respectively, with $\sigma_\tau = 1$ and $\sigma_\gamma = 0.3$.
...
The `sim_activity` function returns a list containing several objects, including the true activity patterns simulated for each of the 100 sites (object: `dat$true_activity_prob`) and the associated realized patterns (i.e. the encounter data; object: `dat$sim_data`). The list also contains the values of the conditional and marginal mean probability of activity (objects: `dat$Conditional` and `dat$Marginal`, respectively) and site-specific phasehsift values (object: `dat$phaseshift`). 
...
```{r est3b, class.source = "fold-hide", fig.cap="Bimodal probability activity curves showing variability in both the frequency of site-use and variability in timing of activity peaks; displayed are activity curves across sites (dashed line) along with the marginal mean curve (black line) and conditional mean curve (red line)."}
...
We see there is considerable variability in the frequency of use among the different sites and relatively smaller variation in timing of peak activity within the 24-hour cycle.

To create Figure 1 in @Iannarilliea2024, we bring together this plot and the two produced in the previous chapter.
...
We can use this dataset for our analysis as it is, considering `y` as our response variable. Alternatively, we can reduce model fitting computational time by counting the number of hourly-long occasions in which the species was observed (i.e. successes) and the number of occasions in which the species was not observed (i.e. failures) for each combination of hourly time interval (e.g., `time = 1`) and camera site (e.g. `id = A1`).
...
In this version (labeled as equation 2 in @Iannarilliea2024), we can model activity patterns using any R package available for fitting GLMMs; we list some of these options in Tables 1 and 2 in @Iannarilliea2024. Here, we choose the `GLMMadaptive` library [@Rizopoulos2022] because it facilitates estimation of both conditional and marginal mean activity patterns (see chapter \@ref(condvsmarg)).

The simplest trigonometric GLMM we can run is one that includes only a random intercept. In the context of estimating activity patterns, a random intercept-only model accounts for variability in the frequency of site-use (i.e. vertical shifts among the curves), but not variability in the timing of activity peaks (i.e. horizontal shift).
...
For trigonometric models, it is difficult to interpret the coefficients. An easier and effective way to explore these results is to visually check the activity patterns predicted based on the model itself. Thus, we predict the estimated (conditional mean) activity patterns, backtrasform the results from logit to the probability scale, and plot the estimated activity pattern along with the true simulated (conditional mean) activity patterns.
...
```{r, class.source = "fold-hide", fig.cap="Comparison of simulated and estimated probability of activity conditional mean curve with variability in the frequency of site-use and timing of peak activity from a trigonometric hierarchical model; shading corresponds to 95% confidence intervals."}
...
This model corresponds, in general terms, to the trigonometric random intercept-only model we ran earlier.  Thus, it only accommodates variability in the frequency of site use and not in the timing of activity peaks. The model again captures the general shape of the simulated conditional mean activity curve. <!---, but the mean activity estimated for the first peak is quite low compared to the simulated pattern. JF - I don't see this; BDG- also not seeing this--->

We can add complexity and include a smoother that shrinks the site-specific estimates toward a common smooth. This model structure resembles the random slope component in trigonometric random intercept and random slope models and allows the site-specific estimates to vary also in the timing of activity peak. In *mgcv*, there is no need to explicitly force the independence between the random intercept and the random slope (as done before for the trigonometric HMs) because there is no covariance term between two random effects.
...
```{r est11b, class.source = "fold-hide", fig.cap="Comparison of simulated and HGAM estimated probability of activity conditional mean curves with variability in the frequency of site-use and timing of peak activity; shading corresponds to 95% confidence intervals."}
# plot
...
Adding a component that allows for variability in timing of peak activity seems to improve the model fit and provides an estimated activity pattern that more closely resembles the one used to simulated the data.

**Going forward**. Throughout the tutorial, we fit trigonometric GLMMs using a binomial distribution (`family = "binomial"` in the code); however, we can run these models also using a Poisson distribution with a few adjustments to data and model structures. We report an example in chapter \@ref(poisson). When working with large datasets, using a Poisson distribution might result in lower computational times.  
