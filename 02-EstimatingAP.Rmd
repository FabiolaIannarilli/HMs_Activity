...
Activity patterns are currently estimated primarily using Kernel Density Estimators (KDEs). However, this approach presents several limitations that we explain in details in @Iannarilliea2024 (Introduction). In this chapter, we introduce two alternative, model-based approaches to estimate activity patterns from time stamped data:
...
To keep the focus on estimating activity patterns, we encapsulate the code to simulate the data in the function *sim_activity*, that we source after loading the necessary libraries. The code in this function is based on equation 1 (chapter \@ref(var) and Materials and Results in @Iannarilliea2024) and its full description, including a description of its arguments and outputs, is provided in chapter \@ref(app).  

```{r est1, message = TRUE, warnings = TRUE}
...
We simulate site-specific bimodal activity patterns for 100 sites (M) and 30 days (J) using the parameters defined in the code below. We include both variability in frequency of site use and variability in timing of activity peaks by setting $\tau_i$ and $\gamma_i$ as drawn from the distributions $\tau_i \sim N(0,\sigma_\tau)$ and $\gamma_i \sim N(0,\sigma_\gamma)$, respectively, with $\sigma_\tau = 1$ and $\sigma_\gamma = 0.3$.
...
The *sim_activity* function returns a list containing several objects, including the true activity patterns simulated for each of the 100 sites (object: *dat$true_activity_prob*) and the associated realized patterns (i.e. the encounter data; object: *dat$sim_data*). The list also contains the values of the conditional and marginal means probability of activity (objects: *dat$Conditional* and *dat$Marginal*, respectively) and site-specific pahasehsift values (object: *dat$phaseshift*). 
...
There is great variability in the frequency of use among the different sites, showed by differences among the curves along the y-axis. There is also a relatively smaller variation in timing of peak activity, with a few sites showing peaks in activity at different times of the 24-hour cycle.
...
We can use this dataset for our analysis as it is, considering _y_ our response variable. Alternatively, we can reduce the computational time by counting the number of hourly-long occasions in which the species was observed (i.e. successes) and the number of occasion in which the species was not observed (i.e. failures) for each combination of hourly time interval (e.g., *time = 1*) and camera site (e.g. *id = A1*).
...
In this new version of the dataset, each row specifies how many time a species was (i.e. *success*: 0 days) and was not (i.e. *failure*: 30 days) detected at a certain site (*Site = A1*) during a certain time interval (*Time = 0*, that is from 00:00 to 00:59). We can now use this dataframe to estimate the diel activity patterns using hierarchical models.

## Trigonometric GLMMs

Equation 1 in chapter \@ref(var) is non-linear due the presence of the phaseshift parameters. Currently there is no out-of-the-box option in R to code trigonometric non-linear model using a frequentist approach. To overcome this challenge, we rewrite equation 1 using compound angle formulas (see also main text @Iannarilliea2024): 

$$
logit(p_t) = \beta_0 + \alpha_1 \times cos(\frac{2\pi t}{24}) + \alpha_2 \times sin(\frac{2\pi t}{24}) + \alpha_3 \times cos(\frac{2\pi t}{12}) + \alpha_4 \times sin(\frac{2\pi t}{12}) + \tau\_i
$$ 
where 

$\alpha_1 = \beta_1 \times cos(\theta_0 + \gamma_i)$, $\alpha_2= -\beta_1 \times sin(\theta_0 + \gamma_i)$, $\alpha_3= \beta_2 \times cos(\theta_1 + \gamma_i)$, and $\alpha_4= -\beta_2 \times sin(\theta_1 + \gamma_i)$.

In this version (labeled as equation 2 in @Iannarilliea2024), we can model activity patterns using any packages available to fit GLMMs; we list some of these options in Tables 1 and 2 in @Iannarilliea2024. Here, we choose the *GLMMadaptive* library [@Rizopoulos2022] because it is currently the only option that allows to also explore conditional and marginal mean activity patterns (see chapter \@ref(condvsmarg)).
...
For trigonometric models, interpreting the model's results looking at the coefficient estimates could be daunting. An easier and effective way to explore these results is to visually check the activity patterns predicted based on the model itself. Thus, we predict the estimated (conditional) activity patterns, backtrasform the results from logit to the probability scale, and plot the estimated activity pattern along with the true simulated (conditional) activity patterns.
...
As before, we visually inspect the (conditional) estimated activity pattern based on this model and compare it with the simulated (conditional) mean activity pattern.
...
**Going forward**. Throughout the tutorial, we fit trigonometric GLMMs using a binomial distribution (_family = "binomial"_ in the code); however, we can run these models also using a Poisson distribution with a few adjustments to data and model structures. We report an example in chapter \@ref(poisson). When working with large datasets, using a Poisson distribution might led to a gain in computational time.  
