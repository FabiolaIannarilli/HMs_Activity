...
To keep the focus on estimating activity pattern, we encapsulate the code to simulate the data in the function *sim_activity*, that we source after loading the necessary libraries. The function simulates true and realized site-specific activity patterns and conditional and marginal mean activity patterns. It is based on equation 1 which is introduced in section \@ref(var) of this tutorial and in the main text (@Iannarilliea2024). A full description of the *sim_activity* function and its arguments is provided in section \@ref(app).
...
We simulate bimodal activity patterns for 100 sites (M) using the parameters defined in the code below. We set $\tau_i$ and $\gamma_i$ as drawn from the distributions $\tau_i \sim N(0,\sigma_\tau)$ and $\gamma_i \sim N(0,\sigma_\gamma)$, respectively, with $\sigma_\tau = 1$ and $\sigma_\gamma = 0.3$. 
...
The *sim_activity* function returns a list containing several objects, including the true activity patterns simulated for each of the 100 sites and the associated realized patterns (i.e. the encounter data). The list also contains the values of the conditional and marginal means probability of activity. We can access the information for a randomly selected set of these sites and plot their true activity patterns, along with the conditional and marginal mean activity patterns (the red and black curves, respectively, in the following plot). 
...
To optimize computational time, we can reduce the number of rows in the dataset by counting the number of hourly-long occasions in which the species was observed (i.e. successeses) and the number of occasion in which the species was not observed (i.e. failures) for each hourly time interval and camera site.
...
Each row in this dataset specifies how many time a species was (0 times) and was not (30 times) detected at a certain site (A1) during a certain time interval (0, that is from 00:00 to 00:59). We can now use this dataframe to estimate activity patterns using trigonometric Generalized Linear Mixed Models.

## Trigonometric GLMMs

Equation 1 in section \@ref(var) is non-linear due the presence of the phaseshift parameters. Currently there is no out-of-the-box option in R to code non-linear model using a frequentist approach. To overcome this challenge, we rewrite equation 1 using compound angle formulas (see also main text @Iannarilliea2024):
$$
...
$$
where $\alpha_1 = \beta_1 \times cos(\theta_0 + \gamma_i)$, $\alpha_2= -\beta_1 \times sin(\theta_0 + \gamma_i)$, $\alpha_3= \beta_2 \times cos(\theta_1 + \gamma_i)$, $\alpha_4= -\beta_2 \times sin(\theta_1 + \gamma_i)$. 

In this version (labeled as equation 2 in the main text), we can model activity patterns with any packages commonly used to fit GLMMs. Here, we choose the *GLMMadaptive* library \@ref(Rizopoulos2022) because it gives us the option to also explore conditional and marginal mean activity patterns (see section \@ref(condvsmarg)).
...
```
For trigonometric models, interpreting the model's results looking at the coefficient estimates could be daunting. An easier and effective way to explore these results is to visually check the activity patterns predicted based on the model itself. Thus, we predict the estimated activity patterns, backtrasform the results in the probability scale, and plot the estimated activity pattern along with the true simulated conditional activity patterns.
...
We can add complexity and include a smoother that shrinks the site-specific esitmates toward a common smooth. This model structure resembles the random slope component in trigonometric random intercept and random slope models and allows the site-specific estimates to vary also in the timing of peak activity. In *mgcv*, there is no need to explicitly force the independence between the random intercept and the random slope (as done before for the trigonometric HMs) because there is no covariance term between two random effects.
