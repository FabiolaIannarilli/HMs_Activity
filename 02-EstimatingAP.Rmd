...
Activity patterns are currently estimated primarily using Kernel Density Estimators (KDEs). However, this approach presents several limitations that we highlight in @HMsActivity2024. In this chapter, we introduce two alternative, model-based approaches to estimate activity patterns from time-stamped data:
...
To create Figure 1 in @HMsActivity2024, we bring together this plot and the two produced in the previous chapter.
...
In this version (labeled as equation 2 in @HMsActivity2024), we can model activity patterns using any R package available for fitting GLMMs; we list some of these options in Tables 1 and 2 in @HMsActivity2024. Here, we choose the `GLMMadaptive` library [@Rizopoulos2022] because it facilitates estimation of both conditional and marginal mean activity patterns (see chapter \@ref(condvsmarg)).
...
We can add complexity and include a smoother that shrinks the site-specific estimates toward a common smooth and one that allows curves to vary among sites. This model structure resembles the random slope component in trigonometric random intercept and random slope models and allows the site-specific estimates to vary also in the timing of activity. In `mgcv`, there is no need to explicitly force the independence between the random intercept and the random slope (as done before for the trigonometric HMs) because there is no covariance term between two random effects.
