...
Kernel Density Estimators are currently the main approach used to estimate activity patterns from camera-trap data. At the time of writing, we are aware of three packages available in R to estimate activity patterns using KDEs: `overlap` [@RidoutLinkie2009; @R-overlap]; `activity`  [@Rowcliffeea2014; @activity2022]; and `circular`  [@circular2022; @Oliveira-Santos2013]. 
...
```
Next, we re-run the analysis using the HM approach. In particular, we fit a trigonometric random intercept-only model and cyclic cubic spline with a smoother for `Time` that depends on `Season`. Both models account for variability in site-use, but not for variability in timing of activity peaks. 
...
We can compare the two HMs to their relative null model to assess the importance of the variable `Season` on bear's activity patterns.
```{r compkde5}
# trig GLMM random intercept-only
# null model
