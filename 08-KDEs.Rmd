...
Kernel Density Estimators are currently the main approach to estimate activity patterns from camera-trap data. At the time of writing, we are aware of three packages available in R to estimate activity patterns based on the KDE approach. The three packages return similar estimates of activity, with some differences. The *overlap* package \@ref(RidoutLinkie2009) converts estimates so that the area under the curve is equal to one. The *activity* package \@ref(Rowcliffeea2014) has the additional option to estimate 95% confidence intervals using bootstrap approaches; the main function implemented in this package, named activity::fitact, has also an argument to correct for differences in detection probability (e.g., between day and night) while the function activity::solartime enable double-anchoring procedures to adjust the timestamp associated with the different records based on sunrise and sunset times (see \ref(Vazquezea2021) for details). Finally, the circular package \@ref(AgostinelliLund2017) can identify periods of higher levels of activity using an isopleth approach similar to that applied in the analysis of telemetry data to identify core areas in home-range estimates \@ref(Oliveira-Santosea2013).

In this chapter, we illustrate how to estimate activity patterns based on KDEs using these three packages. We show the consequences of not aggregating data when estimating activity patterns in \@ref(dataaggr). As an example, we use camera-trap data of American black bears (*Ursus americanus*) collected in northern-Minnesota between mid-May and mid-July 2017 (@Iannarilliea2020).
...
We aggregate the data as described in \@ref(dataprep)
...
We can now proceed to estimating the activity patterns using each of the three packages available in R. We will first run the code for each package and then visually compare the results of all of them.
