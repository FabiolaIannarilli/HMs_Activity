...
We start by fitting a model that describes a *unimodal* activity pattern. This structure includes only the first of the two cosine terms in equation 1 (which translates in the first and second terms of equation 2), and random intercept and slope as described in section \@ref(est).   
...
We can now proceed to compare the different modul structures and associated hypotheses. We can do this using the Akaike Information Criterion (AIC; @AIC).
